...
// CODE HERE
const sayHelloChangeBack = () => {
    sayHelloButton.style.backgroundColor = '#EFEFEF'
    sayHelloButton.style.color = 'black'
}

sayHelloButton.addEventListener('mouseout', sayHelloChangeBack)
...
const repeatMyParamButton = document.querySelector('#repeat-button')
...
const getAQuery = () => {
    ...
    axios.get('http://localhost:3000/query-test/?testywesty&test2')
    .then(res => {
        console.log(res.data)
    })
    .catch(err => {
...
let sendButton = document.querySelector('#query-button')

sendButton.addEventListener('click',getAQuery)
...
foodForm = document.querySelector('#form')
